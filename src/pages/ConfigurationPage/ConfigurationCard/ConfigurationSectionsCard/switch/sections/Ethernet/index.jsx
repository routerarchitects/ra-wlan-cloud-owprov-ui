...
const LldpTlvSelector = ({ prefix, isDisabled }) => {
...
const EthernetEntryFields = ({ prefix, editing }) => {
        ...
        <Box minW="260px" w="100%">
          <FormControl isRequired isInvalid={Boolean(selectPortsTouched && selectPortsError)}>
            ...
            <ChakraSelect
              ...
              value={(entry?.['select-ports'] ?? []).map((val) => ({
...
const EthernetSection = ({ editing, setSection, sectionInformation, removeSub }) => {
      ...
      if (node !== null) {
        ...
        const newSection = {
          data: {
            ...
            configuration: node.values.configuration,
          },
          isDirty: node.dirty,
          invalidValues,
        };
  ...
  return (
    <Formik
      ...
      innerRef={sectionRef}
      initialValues={{
        name: sectionInformation.data?.name ?? 'Ethernet',
        description: sectionInformation.data?.description ?? '',
        weight: sectionInformation.data?.weight ?? 1,
