...
const PassPointForm = ({ isDisabled, namePrefix, isEnabled, onToggle, lockConsortium }) => {
  const name = React.useCallback((suffix) => `${namePrefix}.${suffix}`, []);
  ...
  const iconCell = React.useCallback(
    (src, fileType) => (
      <Image boxSize={100} mx="auto" my="auto" src={`data,${src}`} alt="New Image" />
    ),
    [],
  );
