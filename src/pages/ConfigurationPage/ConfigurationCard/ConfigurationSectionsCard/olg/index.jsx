...
import MetricsSection from '../ap/sections/MetricsSection';
import { METRICS_SCHEMA } from '../ap/sections/MetricsSection/metricsConstants';
import NatSection from './sections/NatSection';
import { DNAT_RULE_SCHEMA, NAT_RULE_SCHEMA, NAT_SCHEMA } from './sections/NatSection/natConstants';
import RadiosSection from '../ap/sections/RadiosSection';
import { RADIOS_SCHEMA } from '../ap/sections/RadiosSection/radiosConstants';
import ServicesSection from '../ap/sections/ServicesSection';
import { SERVICES_SCHEMA } from '../ap/sections/ServicesSection/servicesConstants';
import ThirdPartySection from '../ap/sections/ThirdPartySection';
import { THIRD_PARTY_SCHEMA } from '../ap/sections/ThirdPartySection/thirdPartyConstants';
import UnitSection from '../ap/sections/UnitSection';
import { UNIT_SCHEMA } from '../ap/sections/UnitSection/unitConstants';
...
const getSanitizedNatConfig = (natConfiguration) => {
  const snatRules = sanitizeNatRules(natConfiguration?.snat?.rules ?? []);
  const dnatRules = sanitizeNatRules(natConfiguration?.dnat?.rules ?? []);
  const natConfig = {};
...
const normalizeNatConfiguration = (natConfiguration, t) => {
  const snatRules = natConfiguration?.snat?.rules;
  const dnatRules = natConfiguration?.dnat?.rules;
  const parsedSnatRules = Array.isArray(snatRules)
    ? snatRules.map((rule) => NAT_RULE_SCHEMA(t, true).cast(rule))
    : [];
  const parsedDnatRules = Array.isArray(dnatRules)
    ? dnatRules.map((rule) => DNAT_RULE_SCHEMA(t, true).cast(rule))
    : [];

  return {
    mode: parsedSnatRules.length > 0 ? 'snat' : parsedDnatRules.length > 0 ? 'dnat' : 'none',
    snat: {
      rules: parsedSnatRules,
    },
    dnat: {
      rules: parsedDnatRules,
    },
  };
};
  ...
  normalizeConfigurations(configurations).map(({ key, config, parsed }) => ({
    ...
    configuration:
      key === 'interfaces'
        ? parsed.interfaces
        : key === 'nat'
          ? getSanitizedNatConfig(parsed[key])
          : parsed[key],
  }));
...
const getNormalizedConfigurationSnapshotFromState = (activeConfigurations, sectionsByKey) =>
  activeConfigurations.map((key) => {
    ...
    return {
      ...
      configuration:
        key === 'nat' ? getSanitizedNatConfig(sectionData?.configuration) : sectionData?.configuration,
    };
  });
...
const ConfigurationSectionsCard = ({ configId, editing, setSections, label, onDelete, defaultConfig }) => {
  ...
  const setConfigSectionsFromArray = (arr, shouldValidate) => {
    ...
    if (newActiveConfigs.includes('nat')) {
      const natData = getConfigurationData(normalizedConfigs, 'nat');
      const natDefault = NAT_SCHEMA(t).cast();
      setNat({
        data: {
          ...natDefault,
          ...natData,
          configuration: normalizeNatConfiguration(natData?.configuration, t),
        },
        isDirty: false,
        invalidValues: [],
        shouldValidate,
        ...
        <Box ml={{base: 0, md: 'auto'}}>
          ...
          <ExpertModeButton
            defaultConfiguration={{
              ...
              nat: {
                ...(nat.data ?? {}),
                configuration: getSanitizedNatConfig(nat?.data?.configuration),
              },
              radios: radios.data,
